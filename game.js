        ...
        {gameState === 'rules' && (
          <div className="text-center max-w-2xl mx-auto">
            <button
              ...
              className="mb-6 sm:mb-8 px-12 sm:px-16 py-5 sm:py-6 bg-gradient-to-b from-stone-700 via-stone-800 to-stone-950 text-white text-xl sm:text-2xl font-bold font-serif rounded-2xl hover:from-stone-600 hover:via-stone-700 hover:to-stone-900 transition-all duration-300 shadow-2xl transform hover:scale-105"
              style={{
            ...
            >
              <div className="grid grid-cols-5 gap-0">
                {[...Array(25)].map((_, i) => {
                  ...
                  const isTopRow = row === 0 && col !== 2;
                  const isBottomRow = row === 4 && col !== 2;
            ...
            <div className="bg-black bg-opacity-40 backdrop-blur-md px-6 sm:px-10 py-4 sm:py-6 rounded-2xl shadow-xl">
              <p className="text-stone-200 font-serif text-sm sm:text-base leading-relaxed">
                5×5の盤面で駒を動かし、中央の金色マスを目指すゲームです。駒は前後左右に滑り、端か他の駒に当たるまで止まれません。先に中央マスに止めた方が勝ちです。
              </p>
            </div>
          </div>
        )}
            ...
            <div className="mt-6 sm:mt-8 text-center bg-black bg-opacity-40 backdrop-blur-md px-4 sm:px-6 py-3 sm:py-4 rounded-2xl shadow-xl max-w-sm sm:max-w-lg">
              ...
              {selectedPiece && !winner && (
                <div className="text-yellow-300 font-bold mt-2 sm:mt-3 font-serif drop-shadow-lg text-xs sm:text-sm">
                  <p>移動したい方向のマスをタップ または 矢印キーで方向指定</p>
                  <p className="text-xs mt-1 text-stone-400">ESCキーまたは駒を再タップで選択解除</p>
                </div>
              )}
            </div>
          </div>
